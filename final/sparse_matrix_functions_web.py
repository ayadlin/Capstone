...
with open("source_dict.pickle", "rb") as source:
        source_dict = pickle.load(source)

with open("hyperlink_dict.pickle", "rb") as source:
        hyper_dict = pickle.load(source)
...
def provide_evidence(gene, drug, max_number, r_s, data):
    #gene, drug, max_number, r_s = get_user_input() #, original_indeces
    if r_s == 'r' or r_s =='R':
        original_indeces = data_frame_creator.open_pickle('original_indices_resist.pickle')
    elif r_s == 's' or r_s == 'S':
        original_indeces = data_frame_creator.open_pickle('original_indices_sensit.pickle')
...
def get_evidence_sentences(gene, drug, r_s, max_number,data,original_indeces):
    ...
    for index in index_evidence:
        if len(evidence_list) < max_number and len(evidence_list) < len(index_evidence):
            ...
            #if len(sent)<500:
            doc=doc_names[index]
            source = hyper_dict[doc[:-4]]
            evidence_list.append((source ,sent))
            #else:
