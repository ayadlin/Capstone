    ...
    if genes == 'all':
        ...
        for id in gene_list:
            try:
                ...
                pass#print("Gene: {} not in list".format(id))
...
def get_user_input(genes, kind, path, lab):
    #genes = input('For what genes would you like to get drug interaction information?:, enter "all" for full network  ')
...
def make_graph_interactive(genes, kind, path, lab):
    genes_list = process_genes(genes)
    if lab == 'y' or lab == 'Y':
        labels = True
    else:
...
def make_display_network(lst, path='network', labels=True):
    ...
    if len(lst) ==1:
        ...
        plt.axis('off')
        plt.savefig("static/img/" + path + ".png") # save as png
        plt.show() # display
        Graphs = G
        nx.write_gml(G, "static/img/" + path + '.gml')
        plt.close(1)
